...
# conda.env = "<YOUR_CONDA_ENV_FOR_R>"  # replace
...
get_match_table = function(events1, events2, threshold=0.6, method="lv") {
    ...
    if(method=="embedding_cosine") {
        ...
        source_python(paste0(getwd(),"distance_helper.py"))
    ...
    # # v1 'matches' v2
    # error.rate = (apply(dists,1,min) / nchar(events1))
    # # inds = error.rate < threshold
    # inds = !is.na(apply(dists,1,min)) #rep(T,nrow(dists))
    # mapinds = apply(dists[inds,],1,which.min)
    # # print(dists)
    # # print(mapinds)
    # return(
    #     tibble(v1 = events1[which(inds)], v2 = events2[mapinds],
    #         error.rate = error.rate[inds],
    #         keep = error.rate[inds]<threshold
    #     ) %>% arrange(error.rate)
    # )
}
