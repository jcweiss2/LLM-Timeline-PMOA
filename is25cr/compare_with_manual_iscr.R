...
setwd("<YOUR-CODE-DIR/>")  # used to source "comparer.R" and "distance_helper.py"
man.loc = "<YOUR-MANUAL-ANNOTATION-FOLDER/>"  # with a '/' at the end
ref.locs = 
    c(
        "<YOUR-LLM-ANNOTATION-FOLDER-1/>",
        "<YOUR-LLM-ANNOTATION-FOLDER-2/>",
        "<YOUR-LLM-ANNOTATION-FOLDER-3/>"
    )
conda.env = "<YOUR_CONDA_ENV_FOR_RETICULATE>"  # in addition to the libraries (tidyverse, glmnet, survival, stringdist, reticulate), you need a conda env that can load these python packages: (numpy, transformers, torch, scikit-learn, pandas)
...
flags = list(
            ...
            pilot.names = c("o1-preview", "GPT-4", "GPT-4 w/feedback")
        )
### END PARAMS ###
...
if(flags$event.match) {
    source(paste0(getwd(),"comparer.R"))
    match.tbl = 
...
#     bind_rows(
#         read_csv("/tmp/RtmpC2fc1Y/matches/yourconsensusfile.csv") %>% mutate(time=as.character(time)) %>%
#         filter(pilot.names!="yourconsensusmethod")
#     ) %>% 
